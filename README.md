...
- 点击侧栏上传按钮，选择 PDF 文件
- 系统自动启动 PageIndex 索引流程，构建文档树结构
- 索引完成后会自动进行文档分析，生成摘要和推荐问题

### 2. 对话问答

- 选择已索引的文档，在聊天框输入问题
- Agent 会自动执行：问题分解 → ReAct 循环（多轮搜索/阅读） → 生成回答 → 自我反思
- 推理过程实时展示在界面上

### 3. 查看溯源

- 回答中的节点标签可点击查看对应内容
- 支持跳转到 PDF 页面图像，高亮显示来源位置

### 4. 切换模式

- 通过界面模式切换按钮在文本/视觉模式间切换
- 视觉模式会将页面渲染为图像发送给多模态模型

### 5. 管理技能

- 通过界面中的技能管理面板查看/编辑/启用/禁用技能
...
└────────┬────────┘
   ...
   │  └───┬────┘ │
   │  ┌───▼────┐ │
   │  │  Act   │ │ ← 调用工具（tree_search / read_node / ...）
   │  └───┬────┘ │
   │  ┌───▼─────┐│
   │  │ Observe ││ ← 收集工具返回的信息
   │  └─────────┘│
...
| 文档规模 | 预计 LLM 调用次数 | 使用 gpt-4o-mini | 使用 gpt-4o |
|----------|-------------------|-----------------|-------------|
| 短文档（~10 页） | 30–60 次 | $0.01–0.04 | $0.20–0.60 |
| 论文（~20 页） | 50–100 次 | $0.02–0.08 | $0.40–1.20 |
| 长文档（~100 页） | 150–400 次 | $0.08–0.30 | $1.50–5.00 |
...
| 场景 | 预计 LLM 调用次数 | 使用 gpt-4o-mini | 使用 gpt-4o / gpt-4.1 |
|------|-------------------|-----------------|----------------------|
| 简单问题（单步检索） | 4–6 次 | $0.003–0.008 | $0.05–0.10 |
| 常规问题（多步推理） | 6–12 次 | $0.005–0.015 | $0.08–0.20 |
| 复杂问题（分解 + 重试） | 12–20 次 | $0.01–0.03 | $0.15–0.40 |
| 视觉模式（含图像输入） | 6–15 次 | — | $0.10–0.50 |
